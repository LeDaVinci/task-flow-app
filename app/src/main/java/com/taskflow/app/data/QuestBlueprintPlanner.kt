...
class QuestBlueprintPlanner(
    ...
    fun plan(
        ...
        return QuestBlueprint(
            ...
            mode = mode,
            title = title,
            description = composeDescription(prototype, mode, durationMinutes),
            reward = composeReward(prototype, difficulty, durationMinutes),
    ...
    private fun composeReward(
        ...
        "boss" -> "Boss 宝箱: ${prototype.rewardSeed} +${50 + durationMinutes} XP"
        "chaotic" -> "${prototype.rewardSeed} +${20 + durationMinutes} XP"
        "chill" -> "${prototype.rewardSeed} +${8 + durationMinutes} XP"
        else -> "${prototype.rewardSeed} +${12 + durationMinutes} XP"
    }
    ...
    private val prototypes = listOf(
        ...
        QuestPrototype(QuestTheme.EXERCISE, "桌边激活", "做 20 次开合跳或深蹲，然后喝一口水", "备忘录", "一次动作完成记录", "热启动值"),
        QuestPrototype(QuestTheme.EXERCISE, "楼梯回合", "上下楼梯或原地踏步一轮，直到呼吸明显变快", "备忘录", "一条体感记录", "行动值"),
        QuestPrototype(QuestTheme.MEDITATION, "呼吸计时", "闭眼专注呼吸，默数 20 次吸气和呼气", "备忘录", "一句当前状态", "平静值"),
        QuestPrototype(QuestTheme.MEDITATION, "身体扫描", "从肩膀到脚趾缓慢扫描一遍身体紧张点", "备忘录", "一个最紧的部位", "稳定值"),
        QuestPrototype(QuestTheme.MEDITATION, "环境静听", "什么都不做，只听周围的 3 种声音", "备忘录", "三种声音名字", "降噪值"),
        QuestPrototype(QuestTheme.OUTDOOR, "楼下巡航", "下楼走一圈，找一个今天以前没认真看过的角落", "照片或备忘录", "一张照片或一句记录", "探索值"),
        ...
        QuestPrototype(QuestTheme.WRITING, "一句日记", "只写一句今天最值得留下来的句子", "备忘录", "一句日记", "记录值"),
        ...
        QuestPrototype(QuestTheme.SOCIAL, "具体问候", "给一个人发一句问候，只能提一个具体细节", "聊天窗口", "一条已发送消息", "连接值"),
        QuestPrototype(QuestTheme.SOCIAL, "单句夸奖", "给一个人发一句不求回复的夸奖", "聊天窗口", "一条已发送消息", "勇气值"),
        QuestPrototype(QuestTheme.SOCIAL, "重新开线", "给一个很久没联系的人发一句轻量近况", "聊天窗口", "一条已发送消息", "社交回血"),
        QuestPrototype(QuestTheme.NATURE, "天空取样", "看天空或窗外 2 分钟，记下颜色和形状变化", "备忘录", "两个观察词", "天气值"),
        QuestPrototype(QuestTheme.NATURE, "风的证词", "找一个有风或光线变化的位置站一会儿", "备忘录", "一句自然感受", "感官值"),
        ...
        QuestPrototype(QuestTheme.TIDY, "抽屉突击", "随机打开一个抽屉，只处理最上层那一层", "现场照片或备忘录", "一个处理前后差异", "空间值"),
        ...
        QuestPrototype(QuestTheme.EXPRESSION, "一句宣言", "写一句今天最真实的态度，不修饰不解释", "备忘录", "一句宣言", "表达值"),
        QuestPrototype(QuestTheme.EXPRESSION, "30 秒录音", "录 30 秒语音，说出你现在最想推进的事", "语音备忘录", "一段录音", "声音值"),
        QuestPrototype(QuestTheme.EXPRESSION, "情绪标记", "给现在的情绪起一个名字，再补一句原因", "备忘录", "一个情绪名和一句原因", "命名值"),
    )
}
